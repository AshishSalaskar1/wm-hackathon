...
| Sprint 1 | 2026-05-12 → 2026-05-23 | Data Ingestion & Profile Indexing      | 9       | 43     | Supply & Demand ingestion live; profile index populated in AI Search |
...
**Total:** 45 stories · 195 points · ~10 weeks
...
**Epic:** E-02 | **Type:** Feature | **Points:** 8 | **Owner:** Team A

As the system, I want to read Supply Excel Dump files from either a local staging folder or Azure Blob Storage and parse each row into a typed supply profile object, then vectorize and index the profiles into AI Search when triggered via an API event, so that the full pipeline is runnable locally without requiring Azure Blob Storage.

**Acceptance Criteria:**

**File Source — Local Staging Folder (dev mode):**

* A `LOCAL_STAGING_PATH` environment variable points to a local directory (e.g., `tests/fixtures/staging/`) used as the file source when `AZURE_STORAGE_CONNECTION_STRING` is not set.
* Excel files placed in the local staging folder are read directly from disk; no Azure Blob Storage access is required in dev mode.
* When `AZURE_STORAGE_CONNECTION_STRING` is set (or `LOCAL_STAGING_PATH` is absent), the reader falls back to Azure Blob Storage (`supply-uploads` container) — no code change required to switch modes.
* `docker-compose.yml` (or `.env.example`) documents `LOCAL_STAGING_PATH` so any developer can configure it immediately.

**Parsing:**

* Each row parsed into a `SupplyProfile` model with all confirmed schema fields (SP0-001).
* Parser is schema-validated: if a required column is missing from the file, a `SchemaValidationError` is raised and logged; missing per-row optional values are set to null rather than causing an exception.
...
* Completely empty rows (all fields null or blank) are skipped with a warning log entry; they do not raise an exception.

**Event-Triggered End-to-End Pipeline:**

* `POST /events/profile` accepts a JSON payload `{ "action": "add", "file_name": "<filename>.xlsx" }` and returns HTTP 202.
* On receipt, the handler reads the named file from the local staging folder (or Blob Storage in production), parses all rows into `SupplyProfile` objects, vectorizes each profile using Azure OpenAI Embeddings, and upserts each profile document into the AI Search index.
* Processing runs synchronously within the request for files up to 1,000 rows; the API returns 202 only after all profiles are indexed.
* If the file is not found in the staging folder, the API returns 404 with `{ "detail": "File <filename>.xlsx not found in staging" }`.
* If vectorization fails for a profile, that profile is logged and skipped; remaining profiles continue to be indexed.
* After the handler completes, the indexed profiles are queryable in AI Search.

**Unit Tests:**

* Valid full row parsed correctly with all 16 fields mapped.
* Sparse row with 2 missing optional fields produces a valid `SupplyProfile` with null fields.
* Completely empty row is skipped; no exception raised.
* Malformed file (non-Excel bytes) raises `SchemaValidationError`.
* File missing a required column (e.g., `EMP_NO`) raises `SchemaValidationError` with the column name in the message.

**Dependencies:** SP0-001, SP0-004, SP0-002, SP0-007 (local dev environment).
...
| SP1-001  | Implement Supply Excel Dump Reader and Row Parser            | S1     | E-02 | 8      | Must      |
